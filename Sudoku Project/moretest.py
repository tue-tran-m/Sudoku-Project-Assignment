...
class Board:
    ...
    def find_empty(self):               #finds empty spaces in the board
        for i in range(BOARD_ROWS):
            for j in range(BOARD_COLS):
                if self.cells[i][j] == 0:
                    return i, j
...
def restart():     #THIS FUNCTION RUNS IF THE USER WISHES TO COMPLETELY RESET THE GAME FROM THE START
    ...
    defboard = Board(screen, WIDTH, HEIGHT, "", 8, 8)
    defboard.board = holderboard
...
def reset():
    ...
    boardinit.draw(screen)
    print(f"hboard : {holderboard}")
    print(f"cboard : {boardinit.board}")
